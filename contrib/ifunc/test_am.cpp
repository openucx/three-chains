...
// UCP AM send flag
// Makes almost no difference on Ookami
#define BENCH_AM_FLAG 0
// #define BENCH_AM_FLAG UCP_AM_SEND_FLAG_EAGER
// #define BENCH_AM_FLAG UCP_AM_SEND_FLAG_RNDV
    ...
    for (int pl_sz_log = 0; pl_sz_log <= 20; pl_sz_log++) {
        ...
        for (size_t i = 0; i < n_iters + n_warmup; i++) {
            ...
            sp = ucp_am_send_nb(ep, 0, payload, 1UL << pl_sz_log,
                                ucp_dt_make_contig(1), am_send_cb, BENCH_AM_FLAG);
            request_wait(sp);
        }
