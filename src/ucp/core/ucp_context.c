...
static void ucp_free_resources(ucp_context_t *context)
{
     ...
     */
    for (k = kh_begin(context->ifuncs_map); k != kh_end(context->ifuncs_map);
         k++) {
        if (context->ifuncs[k] == NULL) {
            /* Already de-registered by the user */
            continue;
        }
#ifdef HAVE_LLVM
        free(context->ifuncs[k]->bcarc);
        free(context->ifuncs[k]->deps);
        kh_destroy(ifunc_ep_set_t, context->ifuncs[k]->bc_sent);
#endif  /* HAVE_LLVM */
        free(context->ifuncs[k]->code);
        kh_destroy(ifunc_ep_set_t, context->ifuncs[k]->so_sent);
        free(context->ifuncs[k]);
    }
    kh_destroy(ifunc_map_t, context->ifuncs_map);

#ifdef HAVE_LLVM
    err = LLVMOrcDisposeLLJIT(context->jit);
    ucs_assert(!err);
                                       ...
                                       const ucp_config_t *config)
{
    ...
    err = LLVMOrcCreateLLJIT(&context->jit, builder);
    ucs_assert(err == NULL);

    context->jit_dlr = LLVMOrcLLJITGetMainJITDylib(context->jit);
    ucs_assert(context->jit_dlr != NULL);

    /* Enable symbol resolution in the JIT. */
    err = LLVMOrcCreateDynamicLibrarySearchGeneratorForProcess(
            &sym_gen,
            LLVMOrcLLJITGetGlobalPrefix(context->jit),
            orcjit_allow_symbols,
            NULL);
    ucs_assert(err == NULL);
    (void)err;  /* Supress -Werror when ucs_assert() is disabled. */
    LLVMOrcJITDylibAddGenerator(context->jit_dlr, sym_gen);

    ucs_info("IFUNC initialized LLVM ORCJIT\n");
#endif  /* HAVE_LLVM */
