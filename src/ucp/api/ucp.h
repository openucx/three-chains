...
/**
 * @ingroup UCP_CONTEXT
 * @brief Registration parameters of an ifunc library.
 */
typedef struct {
    /**
     * Name of the ifunc.
    ...
    /**
     * True means this ifunc does not use external symbols.
     */
    int pure;

    /**
     * True means this ifunc is compiled as LLVM bitcode.
     */
    int llvm_bc;
 ...
 * @param [in]  param       Registration parameters, see @ref ucp_ifunc_reg_param_t.
 ...
 * @ingroup UCP_COMM
 *
 * @brief Fills a pointer with the address the payload of the ifunc message.
 *
 * Fills the user-provided pointer with the address of the payload of the
 * provided ifunc message. 
 * 
 * @param [in]  msg_p               Pointer to an ifunc message container.
 * @param [out] payload             Pointer to a pointer to the payload of the ifunc message.
 * @param [out] payload_size        Pointer to size of the payload section. Can be null
 *
 * @return UCS_OK or an error if not able to compute the payload address.
 */
 ...
 * @param [in] worker       UCP worker handle.
 *
 * @return Error code as defined by @ref ucs_status_t
 *
...
/**
 * @ingroup UCP_CONTEXT
 * @brief A in-memory bitcode, its target triple and its size.
 */
typedef struct {
    /**
     * Target triple.
    ...
    /**
     * Pointer to the bitcode buffer.
     */
    void *bc;

    /**
     * Size of the bitcode buffer.
     */
    uint32_t size;
} ucp_llvm_bc_t;
 ...
 * @ingroup UCP_CONTEXT
 *
 * @brief Register an ifunc with in-memory bitcode & deps.
 *
 * This routine searches the directory defined by UCX_IFUNC_LIB_DIR for an ifunc
 * dynamic library named "(*ifunc_name).so", then it loads the library and
 * searches for user-provided ifunc routines and then creates an ifunc handler.
 ...
 * @param [in]  context     Handle to the UCP application context.
 * @param [in]  param       Registration parameters, see @ref ucp_ifunc_reg_param_t.
 * @param [in]  bcs         An array of @ref ucp_llvm_bc_t.
 * @param [in]  n_bcs       Length of the array.
 * @param [in]  deps        A list of dynamic library dependencies.
 * @param [in]  n_deps      Length of the list.
 * @param [out] ifunc_p     A pointer to an ifunc handle.
 *
 * @return UCS_OK or an error if registration has failed.
 */
